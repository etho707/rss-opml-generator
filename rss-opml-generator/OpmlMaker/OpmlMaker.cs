...
namespace OpmlMaker
{
    public static class OpmlMaker
    {
        public static List<string> MakeOpmlStringList(List<OpmlFolder> folders)
        {
            var lines = new List<string>();
            lines.Add("<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
            lines.Add("<opml version=\"1.0\">");
            lines.Add("<head>");
            lines.Add("<title>Generated OPML file for RSS reader</title>");
            lines.Add("</head>");
            lines.Add("<body>");

            lines.AddRange(folders.SelectMany(x => x.ToStringList()));
